...
PBRFragment samplePBR() {
    ...
    vec4 metalRoughNormal = sampleSparse(textures.metalRoughNormal, IN.texCoord, maxLod);


    //rebuild normals
    vec3 normal = vec3(metalRoughNormal.zw, 0);
    ...
    PBRFragment f;
    f.albedo = albedo.xyz;
    f.metalicity = metalRoughNormal.x;
    f.roughness = metalRoughNormal.y;
